...
import {DeliveryBx, Logo, Notification, Search} from '../Assets'
    ...
    return (
        <div className="flex justify-between items-center py-5 space-x-8 relative" style={{zIndex: 10}}>
            ...
            <div className="flex items-center space-x-4  ">
                ...
                <div className="bg-bgLogo w-10 h-10 flex items-center justify-center rounded-full relative"
                     onClick={() => setOpenCart(!isOpenCart)}>
                    <CiShoppingCart fontSize={25}/>
                    <div
                        className="absolute -top-4 w-5 left-6 flex items-center justify-center h-5 rounded-full text-xs text-white bg-primary">
                ...
                <div className="relative" onMouseLeave={() => seOpenAuthDropdown(false)}>
                    {auth ? (
                        <img onMouseOver={() => seOpenAuthDropdown(true)} className="w-9 h-9 rounded-full object-cover"
                             src={auth.avatar} alt=""/>
                    ) : (
                        <Link to="/join/login">
                    ...
                    {auth && isOpenAuthDropdown &&
                        <AuthDropdown auth={auth} onMouseLeave={() => seOpenAuthDropdown(false)}/>}
